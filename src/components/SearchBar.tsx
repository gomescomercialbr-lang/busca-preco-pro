...
export default function SearchBar({ onSearch }: SearchBarProps) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4">
      ...
      {showFilters && (
        <div className="max-w-2xl mx-auto flex items-center justify-center gap-4 animate-in slide-in-from-top-1 duration-300 border-t border-zinc-100 dark:border-zinc-800/50 mt-2 pt-2">
          <div className="flex items-center gap-2">
            <label className="text-[9px] font-black uppercase text-zinc-400">UF:</label>
            <input
              type="text"
              placeholder="SP"
              value={uf}
              ...
              maxLength={2}
              className="bg-transparent border-b border-zinc-200 dark:border-zinc-800 w-10 text-center py-1 text-xs font-bold outline-none focus:border-blue-500 transition-colors"
            />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-[9px] font-black uppercase text-zinc-400">Cidade:</label>
            <input
              type="text"
              placeholder="Nome da cidade..."
              value={city}
              onChange={(e) => setCity(e.target.value)}
              className="bg-transparent border-b border-zinc-200 dark:border-zinc-800 w-40 py-1 text-xs font-bold outline-none focus:border-blue-500 transition-colors"
            />
          </div>
        </div>
      )}
